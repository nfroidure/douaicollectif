...
export async function generateMetadata(props: {
  ...
  const title = `Actualités${page && page !== 1 ? ` - page ${page}` : ""}`;
  const description = "Découvrez les actualités de Douai Collectif.";

  const metadata = await buildMetadata({
    pathname: `/actualites${page && page !== 1 ? `/pages/${page}` : ""}`,
    title,
    description,
    image: {
      url: "/images/actualites.png",
  ...
  return {
    ...
    alternates: {
      ...
      types: {
        ...
        "application/rss+xml": [
          { url: "/actualites.rss", title: `${title} (RSS)` },
        ],
        "application/atom+xml": [
          { url: "/actualites.atom", title: `${title} (Atom)` },
        ],
      },
    },
  };
...
export default async function Page(props: {
  ...
  const baseListingMetadata = entriesToBaseListingMetadata(
    await readEntries<BlogPost>(pathJoin(".", "contents", "actualites")),
  );
